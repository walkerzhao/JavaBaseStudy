...
public class Chain extends Task {
    ...
    public void execute() throws Pausable{
        while(true) {
        	System.out.println("task id:"+ Task.getCurrentTask().id);
            StringBuffer sb = mymb.get();   //阻塞task
            if (nextmb == null) {
            	System.out.println("next mb is null"+Task.getCurrentTask().id);
                System.out.print(sb);
                System.out.println("world");
                System.exit(0);
            } else {
            	System.out.println("next mb is not  null"+Task.getCurrentTask().id);
                sb.append("hello ");
                nextmb.put(sb);
            }
        }
    ...
    public static void main(String args[]) {
        ...
        nextms.putnb(new StringBuffer());  //mb中put一个空的字符串      
    }
}
